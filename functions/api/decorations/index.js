...
export async function onRequestPost({ env, request }) {
  ...
  try {
    const headRes = await runComposite(env, head, "decoration create");
    if (!headRes.ok) {
      return Response.json(
        {
          error: "create_failed",
    ...
    const tailRes = await runChunked(env, tail, { label: "decoration items", refPrefix: "i" });
    if (!tailRes.ok) {
      ...
      await rollbackCreated(env, [...strandedItems, ...created], "decoration create");
      return Response.json(
        {
          error: "create_failed",
